...
using QrCode.Client.Infrastructure.Managers.Misc.Document;
using QrCode.Domain.Entities.Misc;
using QrCode.Shared.Constants.Permission;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Components;

namespace QrCode.Client.Pages.Misc
{
    public partial class MyDocuments
    {
        ...
        private async Task LoadData (int pageNumber, int pageSize, TableState state)
        {
            ...
            if (response.Succeeded)
            {
                ...
                var data = response.Data.Where(d=>d.CreatedBy == CurrentUserId);
                var loadedData = data.Where(element =>
                {
                    if (string.IsNullOrWhiteSpace(_searchString))
                        return true;
                    if (element.Owner.Contains(_searchString, StringComparison.OrdinalIgnoreCase))
                        return true;
                    if (element.Requisition.Contains(_searchString, StringComparison.OrdinalIgnoreCase))
                        return true;
